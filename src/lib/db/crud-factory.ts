...
export function createCrud<T extends ZodType>(config: CrudConfig<T>) {
    ...
    const idSchema = z.union([z.string(), z.coerce.number()]);

    // Use first field as primary key column name
    const pkColumn = fields[0] || 'id';
    ...
    // FORM - Delete record
    const deleteRecord = form(schema as any, async (data: any) => {
        const id = data[pkColumn];
